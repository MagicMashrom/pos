...
class User extends CI_Controller {
    ...
    public function add()
    {
        ...
        $this->form_validation->set_message('required', '%s masih kosong, silakan di isi');
    ...
    public function ubah_password($id)
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($query->num_rows() > 0){
                $data['row'] = $query->row();
                $this->template->load('template', 'user/profile_ubah_password', $data);
            } else {
                echo "<script> 
                    alert('Data tidak ditemukan');";
                echo "window.location='".site_url('user/profile')."';
                </script>";
            }
